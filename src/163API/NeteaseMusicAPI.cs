...
namespace Music163API
{
    public class NeteaseMusicAPI
    {
        ...
        public async Task<SongRoot> GetSongsUrl(long[] song_id, long bitrate = 999000)
        {
            ...
            string raw = await CURL(url, Prepare(JsonConvert.SerializeObject(data)));

            var deserialedObj = JsonConvert.DeserializeObject<SongRoot>(raw);
            return deserialedObj;
        }

        public async Task<DetailResult> GetDetail(long song_id)
        {
        ...
        public async Task<AlbumDetail> GetAlbum(long album_id)
        {
            ...
            string raw = await CURL(url, Prepare(JsonConvert.SerializeObject(data)));
            var deserialedObj = JsonConvert.DeserializeObject<AlbumDetail>(raw);
            return deserialedObj;
        }

        public  async Task<LyricResult> GetLyric(long song_id)
        {
         ...
         public  async Task<SearchSongResponse> SearchSongSuggest(string query)
        {
            string url = "http://music.163.com/weapi/v1/search/get";
            ...
            string raw = await CURL(url, Prepare(JsonConvert.SerializeObject(data)));
            var deserialedObj = JsonConvert.DeserializeObject<SearchSongResponse>(raw);
            return deserialedObj;
           
        }

           public  async Task<SearchAlbumResponse> SearchAlbumSuggest(string query)
        {
            string url = "http://music.163.com/weapi/v1/search/get";
            ...
            string raw = await CURL(url, Prepare(JsonConvert.SerializeObject(data)));
            var deserialedObj = JsonConvert.DeserializeObject<SearchAlbumResponse>(raw);
            return deserialedObj;
        ...
        public Dictionary<string, string> Prepare(string raw)
        {
        ...
        // fake curl
        private async Task<String> CURL(string url, Dictionary<string, string> parms, string method = "POST")
        {
            string result;
            using (var wc = new WebClient())
            {
                ...
                var uri = new Uri(url);
                byte[] responsebytes =  await wc.UploadValuesTaskAsync(uri, method, reqparm);
